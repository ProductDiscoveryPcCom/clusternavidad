...
def main():
    ...
    with st.sidebar:
        ...
        clustering_mode = st.selectbox(
            ...
            - **Guiado por productos**: Usa familias de producto como base
            - **Híbrido**: Combina ambos enfoques (recomendado)
            """
        )
    ...
    if uploaded_file is not None:
        ...
        with st.spinner("🔄 Procesando keywords..."):
            ...
            # Nuevo: matching de productos mejorado
        ...
        if cluster_names is None:
            ...
            for cluster_id in df_filtered['cluster_id'].unique():
                ...
                if top_family and top_family != "Sin match":
                    # Buscar emoji de la familia
                    family_emoji = "📦"
                    for fam_id, fam_data in PRODUCTOS_PCCOMPONENTES.items():
                        if fam_data['nombre'] == top_family:
                            family_emoji = fam_data['emoji']
                            break
                    
                    # Combinar familia + keyword principal
                    name = f"{family_emoji} {top_family}: {top_kw[:30]}"
                else:
                    name = f"C{cluster_id}: {top_kw[:40]}"
